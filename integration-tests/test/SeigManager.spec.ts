  ...
  describe('when the token owner are the only depositor of each layer2', function () {
    ...
    describe('when the token owner equally deposit WTON to all layer2s', function () {
      ...
      it('WTON balance of the token owner must be zero', async function () {
        expect(await this.env.wton.balanceOf(tokenOwner1.address)).to.eq('0');
      });
      ...
      it('deposit manager should have deposited WTON tokens', async function () {
        expect(await this.env.wton.balanceOf(this.env.depositManager.address))
          .to.eq(tokenAmount.times(4).toFixed(WTON_UNIT)/*NUM_ROOTCHAINS*/);
      });
        ...
        await Promise.all(this.env.coinages.map(
          async (coinage) => {
            expect(await coinage.balanceOf(tokenOwner1.address))
              .to.eq(tokenAmount.toFixed(WTON_UNIT));
          },
        ));
      });
    ...
    `;*/
          ...
          /*for (const _i in indices) {
            ...
            it(`${i}-th layer2: the tokwn owner should claim staked amount`, async function () {
              ...
              expect(await this.env.depositManager.pendingUnstaked(layer2.address, tokenOwner1.address)).to.eq('0');
              expect(await this.env.depositManager.accUnstaked(layer2.address, tokenOwner1.address)).to.eq('0');
              ...
              expect(await this.env.depositManager.pendingUnstaked(layer2.address, tokenOwner1.address)).to.eq(amount);
              expect(await this.env.depositManager.accUnstaked(layer2.address, tokenOwner1.address)).to.eq('0');
              ...
              expect(await this.env.depositManager.pendingUnstaked(layer2.address, tokenOwner1.address)).to.eq('0');
              expect(await this.env.depositManager.accUnstaked(layer2.address, tokenOwner1.address)).to.eq(amount);
      ...
      describe('when 0-th layer2 commits 10 times', function () {
        ...
        describe('when the token holder tries to withdraw all stakes', function () {
          ...
          it('should withdraw', async function () {
            ...
            expect(depositManagerWtonBalance0.sub(depositManagerWtonBalance1)).to.eq(wtonAmount);
            expect(tokenOwnerWtonBalance1.sub(tokenOwnerWtonBalance0)).to.eq(wtonAmount);
          });
          ...
          it('should re-deposit', async function () {
            ...
            expect(depositManagerWtonBalance1).to.eq(depositManagerWtonBalance0);
            expect(tokenOwnerWtonBalance1).to.eq(tokenOwnerWtonBalance0);
          });
        ...
        describe('when the token holder tries to withdraw 10% of staked WTON 10 times', function () {
          ...
          it('should withdraw', async function () {
            ...
            expect(depositManagerWtonBalance0.sub(depositManagerWtonBalance1)).to.eq(amount.mul(nBN));
            expect(tokenOwnerWtonBalance1.sub(tokenOwnerWtonBalance0)).to.eq(amount.mul(nBN));
          });
        ...
        function behaveWhenPausedOrNot () {
          ...
          it('withdrawal should not be reverted', async function () {
            for (const layer2 of this.env.layer2s) {
              const staked = await this.env.seigManager.stakeOf(layer2.address, tokenOwner1.address);
              if (staked.gt(BigNumber.from(0))) {
                await this.env.depositManager.connect(tokenOwner1).requestWithdrawal(layer2.address, staked);
              }
            }
        ...
        describe('after seig manager is paused', function () {
          ...
          it('seigniorage must not be given', async function () {
            ...
            expect(totTotalSupply2).to.eq(totTotalSupply1);
          });
          ...
          describe('after seig manager is unpaused', function () {
            ...
            it('seigniorage must be given', async function () {
              ...
              expect(totTotalSupply2).to.gt(totTotalSupply1);
            });
          });
        });
      });
